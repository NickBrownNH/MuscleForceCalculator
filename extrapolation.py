...
class Extrapolate_forces():
    ...
    def write_to_file(self, filename, var00, var0, var1, var2, var3, var4, var5, var6, var777, var77, var7, var8, var9, var10, var11, var12, var13, var14, var15, var16, var17, var18, var19, var20, var21, var22, var23, var24, var25):
        with open(filename, 'a') as file:
            ...
            file.write(f'Left Forearm Size: {var18}\n')
            file.write(f'Left Forearm Max Size: {var19}\n')
            file.write(f'Left Upperarm Size: {var20}\n')
            file.write(f'Left Upperarm Max Size: {var21}\n')
            file.write(f'Right Forearm Size: {var22}\n')
            file.write(f'Right Forearm Max Size: {var23}\n')
            file.write(f'Right Upperarm Size: {var24}\n')
            file.write(f'Right Upperarm Max Size: {var25}\n')
    ...
    """
     ...
     # Function to write variables to a file
    def write_to_file(self, filename, var00, var0, var1, var2, var3, var4, var5, var6, var777, var77, var7, var8, var9, var10, var11, var12, var13, var14, var15, var16, var17, var18, var19, var20, var21, var22, var23, var24, var25, var26):
        with open(filename, 'a') as file:
    ...
    def write_to_file(self, filename, var00, var0, var1, var2, var3, var4, var5, var6, var777, var77, var7, var8, var9, var10, var11, var12, var13, var14, var15, var16, var17, var18, var19, var20, var21, var22, var23, var24, var25, var26):
        with open(filename, 'a') as file:
            file.write(f'Left Arm Angle: {var7}\n')
            file.write(f'Left Shoulder Angle (Degrees): {var777}\n')
            file.write(f'Left Arm Force Valid: {var26}\n')
            file.write(f': {var7-var777}\n')
            file.write('-' * 20 + '\n')
    ...
    def set_depth(self):
        try:
            ...
            try:
                ...
                #Theta B
                self.left_bicep_angle = 180 - self.dot_prod_angle(self.left_wrist_xyz, self.left_bicep_xyz, self.left_shoulder_xyz)
                self.right_bicep_angle = 180 - self.dot_prod_angle(self.right_wrist_xyz, self.right_bicep_xyz, self.right_shoulder_xyz)
                ...
                self.left_arm_angle = 180 - self.dot_prod_angle(self.left_wrist_xyz, self.left_elbow_xyz, self.left_shoulder_xyz)
                print("left arm angle: " + str(self.left_arm_angle))
                self.right_arm_angle = 180 - self.dot_prod_angle(self.right_wrist_xyz, self.right_elbow_xyz, self.right_shoulder_xyz)
                ...
                self.left_arm_force = self.calculate_arm_force_in_plane(self.left_shoulder_angle, self.left_arm_angle, self.weight_added, self.left_bicep_angle, self.left_elbow_xyz[0], self.left_elbow_xyz[1], self.left_wrist_xyz[0], self.left_wrist_xyz[1])
                print("left arm force: " + str(self.left_arm_force))

                self.right_arm_force = self.calculate_arm_force_in_plane(self.right_shoulder_angle, self.right_arm_angle, self.weight_added, self.right_bicep_angle, self.right_elbow_xyz[0], self.right_elbow_xyz[1], self.right_wrist_xyz[0], self.right_wrist_xyz[1])
                print("right arm force: " + str(self.right_arm_force))
                



                #THESE ARE PLACEHOLDER CALCULATIONS - DO NOT USE FOR PRACTICAL TESTS
                self.left_arm_force_placeholder = self.calculate_arm_force_placeholder(self.left_shoulder_angle, self.left_arm_angle, self.weight_added, self.left_bicep_angle, self.left_elbow_xyz[0], self.left_elbow_xyz[1], self.left_wrist_xyz[0], self.left_wrist_xyz[1])
                print("left arm force PLACEHOLDER: " + str(self.left_arm_force_placeholder))

                self.right_arm_force_placeholder = self.calculate_arm_force_placeholder(self.right_shoulder_angle, self.right_arm_angle, self.weight_added, self.right_bicep_angle, self.right_elbow_xyz[0], self.right_elbow_xyz[1], self.right_wrist_xyz[0], self.right_wrist_xyz[1])
                print("right arm force PLACEHOLDER: " + str(self.right_arm_force_placeholder))
                ...
                self.left_forearm_size = self.get_distance_left_elbow_to_left_wrist() * self.m_to_mpu_ratio
                self.left_forearm_max_size = self.init_left_elbow_to_wrist_1 * self.m_to_mpu_ratio

                self.left_upperarm_size = self.get_distance_left_shoulder_to_left_elbow() * self.m_to_mpu_ratio
                self.left_upperarm_max_size = self.init_left_shoulder_to_elbow_1 * self.m_to_mpu_ratio
                
                self.right_forearm_size = self.get_distance_right_elbow_to_right_wrist() * self.m_to_mpu_ratio
                self.right_forearm_max_size = self.init_right_elbow_to_wrist_1 * self.m_to_mpu_ratio

                self.right_upperarm_size = self.get_distance_right_shoulder_to_right_elbow() * self.m_to_mpu_ratio
                self.right_upperarm_max_size = self.init_right_shoulder_to_elbow_1  * self.m_to_mpu_ratio


                


                #Condition 1 Checks Whether or Not The Bicep Is Being Used and Whether The Force Should Be Printed Out
                if self.left_arm_angle > self.left_shoulder_angle:
                    self.left_condition_1 = False
                else:
                    self.left_condition_1 = True

                if self.right_arm_angle > self.right_shoulder_angle:
                    self.right_condition_1 = False
                else:
                    self.right_condition_1 = True


                #Condition 1 Checks Whether or Not The Bicep Is Being Used and Whether The Force Should Be Printed Out
                if np.abs(self.is_left_arm_in_plane) < 500:
                    self.left_condition_2 = False
                else:
                    self.left_condition_2 = True

                if np.abs(self.is_right_arm_in_plane) < 500:
                    self.right_condition_2 = False
                else:
                    self.right_condition_2 = True




                #Force Val Checking and Formatting 
                if self.left_condition_1 is True:
                    self.left_force = 0
                elif self.left_condition_2 is True:
                    self.left_force = self.left_arm_force_not_in_plane
                else:
                    self.left_force = self.left_arm_force
                


                if self.right_condition_1 is True:
                    self.right_force = 0
                elif self.right_condition_2 is True:
                    self.right_force = self.right_arm_force_not_in_plane
                else:
                    self.right_force = self.right_arm_force
                ...
                self.write_to_file('values.txt', self.left_hip_xyz, self.right_hip_xyz, self.left_shoulder_xyz, self.right_shoulder_xyz, self.left_elbow_xyz, self.right_elbow_xyz, self.left_wrist_xyz, 
                                   self.right_wrist_xyz, self.left_shoulder_angle, self.right_shoulder_angle, self.left_arm_angle, self.right_arm_angle, self.pitch, self.left_force, self.right_arm_force, 
                                   ...
                                   self.left_forearm_size, self.left_forearm_max_size, self.left_upperarm_size, self.left_upperarm_max_size, self.right_forearm_size, self.right_forearm_max_size, 
                                   self.right_upperarm_size, self.right_forearm_max_size, self.left_condition_1)
                ...
                self.calculated_data = {
                    "right_bicep_force": str("%0.2f" % self.right_force),
                    "right_elbow_angle": str("%0.2f" % self.right_arm_angle),
                    "left_bicep_force": str("%0.2f" % self.left_force),
                    "left_elbow_angle": str("%0.2f" % self.left_arm_angle),
                    "uarm_spher_coords": str(0),
                    "farm_spher_coords": str(0)
                }
                """
                #USE FOR PLACEHOLDER DATA DISPLAY ONLY - NOT ACTUAL VALUES
                # set/return data in dictionary format
                self.calculated_data = {
                    "right_bicep_force": str("%0.2f" % self.right_arm_force_placeholder),
                    "right_elbow_angle": str("%0.2f" % self.right_arm_angle),
                    "left_bicep_force": str("%0.2f" % self.left_arm_force_placeholder),
                    "left_elbow_angle": str("%0.2f" % self.left_arm_angle),
                    "uarm_spher_coords": str(0),
                    "farm_spher_coords": str(0)
                }
    ...
    def calculate_z(self, z_init, max_length, max_length3, actual_length, pitch):
        ...
        act_len = actual_length*self.m_to_mpu_ratio
        #act_len_prime = act_len - (self.depth_ratio*(self.left_side_hip_shoulder_distance*abs(self.pitch_in_degrees/90))) - ((max_len3-max_len1)*abs((90-hip_shoulder_elbow_angle)/90))
        print("k**2: " + str(k**2) + "  max_len**2: " + str(max_len**2) + "  act_len: " + str(act_len) + "  max_len: " + str(max_len) + "  k: " + str(k))
        print("np.sqrt(" + str(k**2) + " * " + str(max_len**2) + " - (" + str(act_len) + " - " + str(max_len) + "))")

        n = np.sqrt(k**2 * max_len**2 - (act_len - max_len)**2)
        act_len_prime = n/k
        ...
        print("\n\n z = z_init - ((Lc'' - Lm) / k) \n" + 
            str(z) + " = " + str(z_init) + " - (" + str(act_len_prime) + " - " + str(max_len) + ") / " + str(k) + "\n\n" +
    ...
    def calculate_z(self, z_init, max_length, max_length3, actual_length, pitch):
        ...
        print("-------------------------------------------------------------------------------------------------------")
        print("k from 19")
        print("fv1: " + str(fv1) + " fv2: " + str(fv2) + " u: " + str(u) + " ump: " + str(ump) + " r: " + str(r) + "\n")
        print("uv1**2: " + str(u**2) + "\n (uv2 * r * (fv1 / fv2))**2: " + str((ump * r * (fv1 / fv2))**2))
        denominator = -np.sqrt(u**2 - (ump * r * (fv1 / fv2))**2)
        k = numerator / denominator
        print("result: " + str(k) + "\n")
        ...
        print("k from 18")
        k2 = (fv2 - fv1) / -u 
        print("fv1: " + str(fv1) + " fv2: " + str(fv2) + " u: " + str(u))
        print("(" + str(fv2) + " - " + str(fv1) + ") / " + str(-u))
        print("result: " + str(k2) + "\n")
        print("-------------------------------------------------------------------------------------------------------")        
       ...
       ""
        ...
        LcPP = actual_length * self.m_to_mpu_ratio
        LcPP_modded = LcPP - (self.depth_ratio*(self.left_side_hip_shoulder_distance*abs(self.pitch_in_degrees/90))) - ((max_length3-max_length)*abs((90-hip_shoulder_elbow_angle)/90))
        Lm = max_length * self.m_to_mpu_ratio
        ...
        print("LcPP: " + str(LcPP) + "  LcPP_modded " + str(LcPP_modded) + "  Lm: " + str(Lm) + "  k: " + str(k) + "  k2: " + str(k2) + "  z_init: " + str(z_init))
        z1 = z_init - ((LcPP - Lm) / k)
        print("z1 = z_init - (LcPP - Lm) / k  |  " + str(z1) + " = " + str(z_init) + " - (" + str(LcPP) + " - " + str(Lm) + ") / " + str(k))
        z2 = z_init - ((LcPP - Lm) / k2)
        print("z2 = z_init - (LcPP - Lm) / k2  |  " + str(z2) + " = " + str(z_init) + " - (" + str(LcPP) + " - " + str(Lm) + ") / " + str(k2))
        z3 = z_init - ((LcPP_modded - Lm) / k)
        print("z3 = z_init - (LcPP_modded - Lm) / k  |  " + str(z3) + " = " + str(z_init) + " - (" + str(LcPP_modded) + " - " + str(Lm) + ") / " + str(k))
        z4 = z_init - ((LcPP_modded - Lm) / k2)
        print("z4 = z_init - (LcPP_modded - Lm) / k2  |  " + str(z4) + " = " + str(z_init) + " - (" + str(LcPP_modded) + " - " + str(Lm) + ") / " + str(k2))
    ...
    def calculate_z(self, z_init, max_length, max_length3, actual_length, pitch):
        ...
        z = z_init - (-k * LcPP + np.sqrt((-LcPP**2) + (Lm**2) + (k**2) * (Lm**2))) / (1 + k**2)

        print("\n z: " + str(z) + "  z_init: " + str(z_init) + "  k: " + str(k) + "  LcPP: " + str(LcPP) + " Lm: " + str((Lm)))


        print("\n z = z_init - (-k * LcPP + np.sqrt((-LcPP**2) + (Lm**2) + (k**2) * (Lm**2))) / 1 + k**2 \n  " + 
              str(z) + " = " + str(z_init) + " + (" + str(-k) + " * " + str(LcPP) + " + sqrt(" + str((-LcPP**2)) + " + " + str((Lm**2)) + " + " + str((k**2)) + " * " + str((Lm**2)) + ")) / 1 + " + str(k**2))

        return z
    """
    """
    
    #Equation 18 - Did not work
    ...
    def calculate_z(self, z_init, max_length, max_length3, actual_length, pitch):
        ...
        print(f"\nk = {k}, d = {d}, LcPP = {LcPP}, z = {z}\n")
        print(f"k = -((fv1 * fv2 * u) / (fv1 - fv2)) = -(({fv1} * {fv2} * {u}) / ({fv1} - {fv2})) = {k}")
        print(f"d = -((fv2 * u) / (fv1 - fv2)) = -(({fv2} * {u}) / ({fv1} - {fv2})) = {d}")
        print(f"LcPP = actual_length * self.m_to_mpu_ratio = {actual_length} * {self.m_to_mpu_ratio} = {LcPP}")
        print(f"z = z_init - (d - (k / LcPP)) = {z_init} - ({d} - ({k} / {LcPP})) = {z}")

        return z
    """
    
    """
    #22 - gives nan (d is bigger than Lm)
    ...
    def calculate_z(self, z_init, max_length, max_length3, actual_length, pitch):
        ...
        z = z_init - ((Lm * (d * (LcPP**2) * Lm + k * np.sqrt((k**2) + (LcPP**2) * ((Lm**2) - (d**2)))))/((k**2) + (LcPP**2) * (Lm**2)))

        print(f"fv2 = self.init_left_elbow_to_wrist_2 * self.m_to_mpu_ratio = {self.init_left_elbow_to_wrist_2} * {self.m_to_mpu_ratio} = {fv2}")
        print(f"fv1 = self.init_left_elbow_to_wrist_1 * self.m_to_mpu_ratio = {self.init_left_elbow_to_wrist_1} * {self.m_to_mpu_ratio} = {fv1}")
        print(f"u = self.user_height_raw * 0.173 = {self.user_height_raw} * 0.173 = {u}")
        print(f"Lm = max_length * self.m_to_mpu_ratio = {max_length} * {self.m_to_mpu_ratio} = {Lm}")
        print(f"k = -((fv1 * fv2 * u) / (fv1 - fv2)) = -(({fv1} * {fv2} * {u}) / ({fv1} - {fv2})) = {k}")
        print(f"d = -((fv2 * u) / (fv1 - fv2)) = -(({fv2} * {u}) / ({fv1} - {fv2})) = {d}")
        print(f"LcPP = actual_length * self.m_to_mpu_ratio = {actual_length} * {self.m_to_mpu_ratio} = {LcPP}")
        print(f"z = z_init - ((Lm * (d * (LcPP**2) * Lm + k * np.sqrt((k**2) + (LcPP**2) * ((Lm**2) - (d**2))))) / ((k**2) + (LcPP**2) * (Lm**2))) = {z_init} - (({Lm} * ({d} * ({LcPP}**2) * {Lm} + {k} * np.sqrt(({k}**2) + ({LcPP}**2) * (({Lm}**2) - ({d}**2))))) / (({k}**2) + ({LcPP}**2) * ({Lm}**2))) = {z}")
    ...
    def calculate_z(self, z_init, max_length, max_length3, actual_length, pitch):
        ...
        z = z_init - ((d * (LcPP**2) * (Lm**2)) / ((k**2) + ((LcPP**2) * (Lm**2))))
        ...
        print(f"d = -((fv2 * u) / (fv1 - fv2)) = -(({fv2} * {u}) / ({fv1} - {fv2})) = {d}")
        print(f"LcPP = actual_length * self.m_to_mpu_ratio = {actual_length} * {self.m_to_mpu_ratio} = {LcPP}")
        print(f"z = z_init - ((d * (LcPP**2) * (Lm**2)) / ((k**2) + ((LcPP**2) * (Lm**2)))) = {z_init} - (({d} * ({LcPP}**2) * ({Lm}**2)) / (({k}**2) + (({LcPP}**2) * ({Lm}**2)))) = {z}")
    ...
    """
    def calculate_arm_force_in_plane(self, thetaUpper, thetaArm, weightAdded, thetaB, xH, yH, xE, yE):
        
        thetaH = np.arctan(xH - xE) / (yH - yE)
        ...
        return force
    """
    def calculate_arm_force_in_plane(self, thetaArm, thetaU, weightAdded, thetaB, xH, yH, xE, yE):
        
        thetaH = np.arctan(xH - xE) / (yH - yE)


        leverArmFA = self.cfg * np.sin(thetaH)
        leverArmAdd = self.forearm * np.sin(thetaH)
        leverArmBic = self.b * np.sin(thetaB)

        print("ThetaB: " + str(thetaB) + "thetaH: " + str(thetaH) + ", leverArmFA: " + str(leverArmFA) + "leverArmAdd: " + str(leverArmAdd) + "leverArmBic: " + str(leverArmBic))
        force = abs((self.weightForearm*9.81 * leverArmFA + weightAdded*9.81 * leverArmAdd) / leverArmBic)
        print("Bicep Force: " + str(force))

        with open("forces", 'a') as file:
            file.write(f'Arm Force: {force}\n')
            file.write(f'force = abs((self.weightForearm*9.81 * leverArmFA + weightAdded*9.81 * leverArmAdd) / leverArmBic)\n')
            file.write(f'{force} = abs(({self.weightForearm*9.81} * {leverArmFA} + {weightAdded*9.81} * {leverArmAdd}) / {leverArmBic})\n')
            file.write(f'leverArmFA = self.cfg * np.sin(thetaH)\n')
            file.write(f'{leverArmFA} = {self.cfg} * {np.sin(thetaH)}\n')
            file.write(f'leverArmAdd = self.forearm * np.sin(thetaH)\n')
            file.write(f'{leverArmAdd} = {self.forearm} * {np.sin(thetaH)}\n')
            file.write(f'leverArmBic = self.b * np.sin(thetaB)\n')
            file.write(f'{leverArmBic} = {self.b} * {np.sin(thetaB)}\n')
            file.write(f'thetaH = np.arctan(xH - xE) / (yH - yE)\n')
            file.write(f'{thetaH} = np.arctan({xH} - {xE}) / ({yH} - {yE})\n')
            file.write(f'cfg: {self.cfg}\n')
            file.write(f'forearm: {self.forearm}\n')
            file.write(f'b: {self.b}\n')
            file.write(f'thetaH: {thetaH}\n')
            file.write(f'thetaB: {thetaB}\n')
            file.write(f'thetaU: {thetaU}\n')
            file.write(f'thetaArm: {thetaArm}\n')
            file.write('-' * 20 + '\n')
        return force
    

    def calculate_arm_force_placeholder(self, thetaArm, thetaU, weightAdded, thetaB, xH, yH, xE, yE):
        "Place Holder Force Calculation - NOT FOR ACTUAL USE - Only theorhetical force values to be displayed when shoulders are at a 90 degree angle"
        force = 0.27*((thetaU-90)**2) - 10 + (random.random()*27)
        print("Bicep Force Placeholder: " + str(force) + "   :   thetaUpper: " + str(thetaU))
